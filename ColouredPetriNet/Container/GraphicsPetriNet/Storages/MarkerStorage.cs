...
namespace ColouredPetriNet.Container.GraphicsPetriNet
{
    public partial class GraphicsPetriNet
    {
        ...
        public delegate void ForEachMarkerFunction(GraphicsItems.GraphicsItem marker);
        ...
        private class MarkerStorage : Interfaces.IMarkerStorage
        {
            ...
            public Interfaces.IMarkerWrapper this[int id]
            {
                get
                {
                    ...
                    {
                        itemList = Markers[i] as Interfaces.IOneTypeItemList;
            ...
            public int GetTypeId<T>()
            {
                ...
                for (int i = 0; i < Markers.Count; ++i)
                {
                    itemList = Markers[i] as OneTypeItemList<MarkerWrapper<T>>;
            ...
            public Interfaces.IOneTypeItemList GetStorage(int type)
            {
                ...
                for (int i = 0; i < Markers.Count; ++i)
                {
                    storage = Markers[i] as Interfaces.IOneTypeItemList;
            ...
            public OneTypeItemList<MarkerWrapper<T>> GetStorage<T>()
            {
                ...
                for (int i = 0; i < Markers.Count; ++i)
                {
                    storage = Markers[i] as OneTypeItemList<MarkerWrapper<T>>;
                    if (!ReferenceEquals(null, storage))
                    {
                        return storage;
                    }
